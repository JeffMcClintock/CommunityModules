...
#endif
	void FileWatcher::Start(const platform_string& fullPath, std::function<void(void)> callback)
	{
#if defined(_WIN32)
		_RPT1(_CRT_WARN, "FileWatcher::Start(%S)\n", fullPath.c_str());
		stopEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
		backgroundThread = std::thread([stopEvent=stopEvent, fullPath, callback] {WatchDirectoryPrivate(stopEvent, fullPath, callback); });
#endif
	}
